...
def parse_uploaded_yaml(uploaded_file) -> dict:
    """
    Read uploaded YAML file (Streamlit UploadedFile) and return dict.
    """
...
def reset_unsafe_fields_for_import():
    """
    Clear anything we never want to prefill from an imported YAML.
    - catalog selection (category/sub/subsub)
    - new category/sub inputs
    - figures
    - flow flags (form_done, ready_for_download)

    NOTE:
    Your catalog selectboxes currently do NOT have keys, so they can't be prefilled by session_state anyway.
    We still keep this for safety + future changes.
    """
    # Flow flags
    st.session_state["form_done"] = False
    st.session_state["ready_for_download"] = False

    # ---- catalog selection: we *intentionally* do NOT prefill ----
    # Only clears keys if you add widget keys later.
    for k in [
        "category_choice",
        "subcategory_choice_raw",
        "subsub_choice_raw",
        "new_category_name",
    ...
    # ---- figures: never prefill ----
    keys_to_remove = [
        k for k in st.session_state.keys()
        if k.startswith("fig_type_") or k.startswith("fig_caption_") or k.startswith("fig_is_cover_")
    ]
    for k in keys_to_remove:
        st.session_state.pop(k, None)


def prefill_safe_fields_from_yaml(data: dict):
    """
    Prefill only safe fields from YAML.
    Does NOT prefill category/sub/subsub or figures.
    """
    if not data:
        return

    # Show imported legacy info (optional; used in UI)
    st.session_state["import_legacy_topic"] = str(data.get("topic") or "").strip()
    st.session_state["import_legacy_cat"] = str(data.get("catalog_category") or "").strip()
    st.session_state["import_legacy_subcat"] = str(data.get("catalog_subcategory") or "").strip()
    st.session_state["import_legacy_subsub"] = str(data.get("catalog_subsubcategory") or "").strip()

    # Straightforward text fields
    st.session_state["resource_title"] = str(data.get("title") or "")

    # -------------------------------------------------
    # Resource type mapping (IMPORTANT)
    # Streamlit selectbox only supports: Streamlit app / Jupyter Notebook / Other
    # Legacy YAMLs might have: Video / Dataset / etc.
    # -------------------------------------------------
    raw_type = str(data.get("resource_type") or "").strip()
    allowed_ui_types = {"Streamlit app", "Jupyter Notebook", "Other"}

    if raw_type in allowed_ui_types:
        mapped_type = raw_type
    elif raw_type.lower() in {"streamlit", "streamlitapp", "streamlit app"}:
        mapped_type = "Streamlit app"
    elif raw_type.lower() in {"jupyter", "jupyter notebook", "notebook"}:
        mapped_type = "Jupyter Notebook"
    elif raw_type:
        mapped_type = "Other"
        st.session_state["import_resource_type_warning"] = (
            f"Uploaded YAML has resource_type='{raw_type}', which is not supported as a direct UI option here. "
            f"It was mapped to 'Other'. You can change it manually."
        )
    else:
        mapped_type = "Other"

    st.session_state["submission_type"] = mapped_type

    st.session_state["access_url"] = str(data.get("url") or "")
    st.session_state["description_short"] = str(data.get("description_short") or "")

    # keywords -> comma separated
    keywords_list = _as_list_of_str(data.get("keywords"))
    st.session_state["keywords_text"] = ", ".join(keywords_list)

    # fit_for -> list
    fit_for_list = _as_list_of_str(data.get("fit_for"))
    st.session_state["fit_for"] = fit_for_list

    # prerequisites
    st.session_state["prereq_text"] = str(data.get("prerequisites") or "")

    # references -> textarea lines
    refs_list = _as_list_of_str(data.get("references"))
    st.session_state["references_text"] = "\n".join(refs_list)

    # time_required
    # Keep it simple: set as "Custom" and fill custom text if it doesn't match presets.
    time_required = str(data.get("time_required") or "").strip()
    presets = {
        "5–15 min",
        "15–30 minutes",
        "30–45 minutes",
    ...
    if time_required in presets:
        st.session_state["time_choice"] = time_required
        st.session_state["time_required_custom"] = ""
    else:
        st.session_state["time_choice"] = "Custom"
        st.session_state["time_required_custom"] = time_required

    # Streamlit-specific toggles/counters (safe even if resource isn't streamlit; you already guard later)
    st.session_state["multipage_app"] = bool(data.get("multipage_app", False))
    st.session_state["num_pages"] = int(data.get("num_pages") or 0)

    st.session_state["interactive_plots"] = bool(data.get("interactive_plots", False))
    st.session_state["num_interactive_plots"] = int(data.get("num_interactive_plots") or 0)

    st.session_state["assessments_included"] = bool(data.get("assessments_included", False))
    st.session_state["num_assessment_questions"] = int(data.get("num_assessment_questions") or 0)

    st.session_state["videos_included"] = bool(data.get("videos_included", False))
    st.session_state["num_videos"] = int(data.get("num_videos") or 0)
    ...
    if isinstance(authors, list) and authors:
        ...
        for a in authors:
            ...
            if name:
                clean.append((name, aff))
        if clean:
            st.session_state["authors_count"] = max(1, min(10, len(clean)))
            for i, (name, aff) in enumerate(clean[:10]):
                st.session_state[f"author_name_{i}"] = name
                st.session_state[f"author_aff_{i}"] = aff


def compute_upload_signature(uploaded_file) -> str:
    """
    Compute a stable signature for an uploaded file to avoid re-importing the same YAML repeatedly.
    """
    if uploaded_file is None:
        return ""
    try:
...
# -------------------------------------------------
# START MODE (TOP OF APP): NEW vs UPLOAD+PREFILL
# -------------------------------------------------


# Ensure start_mode exists
...
with colA:
    if st.button("⬆️ Upload existing YAML (prefill)", use_container_width=True):
...
start_mode = st.session_state.get("start_mode", "new")

if start_mode == "upload":
    uploaded_yaml = st.file_uploader(
        "Upload a YAML file to prefill fields (catalog location + figures will NOT be imported).",
        type=["yaml", "yml"],
    )
    ...
    if uploaded_yaml is not None:
        ...
        # Guard: only import if it's a new/different file than the last import
        if sig and sig != st.session_state.get("last_import_sig"):
            data = parse_uploaded_yaml(uploaded_yaml)
            if not data:
                st.error("Could not read YAML (empty or invalid).")
            else:
                st.session_state["last_import_sig"] = sig
                reset_unsafe_fields_for_import()
                prefill_safe_fields_from_yaml(data)
                st.success("Imported YAML. Now select the current catalog location below and review the fields.")
                st.rerun()

# Optional: show legacy info panel (read-only)
legacy_topic = st.session_state.get("import_legacy_topic", "")
legacy_cat = st.session_state.get("import_legacy_cat", "")
legacy_subcat = st.session_state.get("import_legacy_subcat", "")
legacy_subsub = st.session_state.get("import_legacy_subsub", "")

# IMPORTANT: show an always-visible message so users understand catalog is NOT imported
if start_mode == "upload" and (legacy_topic or legacy_cat or legacy_subcat or legacy_subsub):
    st.info(
        "When importing an existing YAML, the category, subcategory, and sub-subcategory are intentionally not restored to avoid mismatches with the current catalog structure. \n\n"
        "Figures are also not imported and must be re-uploaded if needed. \n\n"
        "All other compatible fields have been prefilled.\n\n"
        " Reload the page to start from scratch.\n\n"

    )


# Show resource_type mapping warning (if any)
if st.session_state.get("import_resource_type_warning"):
    st.warning(st.session_state["import_resource_type_warning"])

if start_mode == "upload" and any([legacy_topic, legacy_cat, legacy_subcat, legacy_subsub]):
    with st.expander("Imported info from YAML (read-only)", expanded=False):
        if legacy_topic:
            st.write(f"**Legacy topic:** {legacy_topic}")
        if legacy_cat or legacy_subcat or legacy_subsub:
            st.write("**Legacy catalog location (will NOT be used automatically):**")
            st.write(f"- Category: {legacy_cat or '—'}")
            st.write(f"- Subcategory: {legacy_subcat or '—'}")
            st.write(f"- Sub-subcategory: {legacy_subsub or '—'}")
...
resource_title = st.text_input("Title of the resource", "", key="resource_title")
...
# --- one-time defaults for Streamlit app fields (avoid value= conflicts) ---
if "multipage_app" not in st.session_state:
    st.session_state["multipage_app"] = False
if "num_pages" not in st.session_state:
    st.session_state["num_pages"] = 2

if "interactive_plots" not in st.session_state:
    st.session_state["interactive_plots"] = False
if "num_interactive_plots" not in st.session_state:
    st.session_state["num_interactive_plots"] = 1

if "assessments_included" not in st.session_state:
    st.session_state["assessments_included"] = False
if "num_assessment_questions" not in st.session_state:
    st.session_state["num_assessment_questions"] = 1

if "videos_included" not in st.session_state:
    st.session_state["videos_included"] = False
...
if submission_type == "Streamlit app":
    ...
    multipage_app = st.checkbox("Is this a multipage Streamlit app?", key="multipage_app")
    if multipage_app:
        num_pages = st.number_input("Approximate number of pages", min_value=1, step=1, key="num_pages")

    interactive_plots = st.checkbox("Does the app contain interactive plots?", key="interactive_plots")
    if interactive_plots:
        num_interactive_plots = st.number_input("Approximate number of interactive plots", min_value=1, step=1, key="num_interactive_plots")

    assessments_included = st.checkbox("Does the app include assessments (questions)?", key="assessments_included")
    if assessments_included:
        num_assessment_questions = st.number_input("Approximate number of assessment questions", min_value=1, step=1, key="num_assessment_questions")

    videos_included = st.checkbox("Does the app include embedded video / tutorials?", key="videos_included")
    if videos_included:
        num_videos = st.number_input("Approximate number of videos", min_value=1, step=1, key="num_videos")
...
# --- one-time defaults (avoid default/index conflicts with YAML import) ---
if "time_choice" not in st.session_state:
    st.session_state["time_choice"] = "15–30 minutes"
if "time_required_custom" not in st.session_state:
    st.session_state["time_required_custom"] = ""
...
if time_choice == "Custom":
    time_required = st.text_input(
        "Custom time description",
        "",
        key="time_required_custom",
    )
else:
...
# Ensure a default only if nothing is set yet
if "fit_for" not in st.session_state:
    st.session_state["fit_for"] = ["self learning"]
